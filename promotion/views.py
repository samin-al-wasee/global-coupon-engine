...
def country_page(request, target_country_slug):
	...
	offers = Offer.objects.filter(offer_expires_on__gte=date.today(), offered_in=target_country)
...
def brand_page(request, target_country_slug, target_brand_slug):
	...
	expired_offers = Offer.objects.filter(offer_expires_on__lte=date.today(), offered_by=target_brand, offered_in=target_country)
